...
<div id="cooked-settings-prewrap"><div id="cooked-settings-wrap">
    ...
    foreach ( $cooked_settings_tabs_fields as $tab_slug => $tab ) :
        echo '<li id="cooked-settings-tab-' . esc_attr( $tab_slug ) . '"' . ( 1 === $tab_counter ? ' class="active"' : '' ) . '><a href="#' . esc_attr( $tab_slug ) . '"><i class="cooked-icon cooked-icon-' . esc_attr( $tab['icon'] ) . '"></i>&nbsp;&nbsp;' . esc_attr( $tab['name'] ) . '</a></li>';
        ++$tab_counter;
        endforeach;
        echo '</ul>';
                    ...
                    foreach ( $cooked_settings_tabs_fields as $tab_slug => $tab ) :
                        ...
                        foreach ( $tab['fields'] as $name => $field ) :
                            ...
                            if ( method_exists( $Cooked_Settings, $field_type ) ) :
                                ...
                                if ( 'misc_button' === $field['type'] || 'nonce' === $field['type'] ) :
                                    $notitle       = true;
    ...
    if ( ! empty( $conditions ) ) :
        ...
        foreach ( $conditions as $setting_name => $array ) :
            foreach ( $array as $name => $true_val ) :
                $val                           = ( isset( $_cooked_settings[ $setting_name ] ) && is_array( $_cooked_settings[ $setting_name ] ) && in_array( $true_val, $_cooked_settings[ $setting_name ], true ) ? 'true' : 'false' );
