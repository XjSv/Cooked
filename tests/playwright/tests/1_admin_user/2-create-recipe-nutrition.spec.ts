...
test.describe('Create a new complete recipe (admin)', () => {
  ...
  test('Edit the recipe (admin)', async ({ adminContext }) => {
    ...
    await Promise.all([
      ...
      adminPage.waitForURL(/post\.php\?post=\d+&action=edit/),
      // Wait for success message
      adminPage.waitForSelector('.notice.notice-success.updated', { timeout: 10000 })
    ]);

    // Check for success message content
    await expect(adminPage.locator('.notice.notice-success.updated')).toContainText('Post updated.');
  });
});
