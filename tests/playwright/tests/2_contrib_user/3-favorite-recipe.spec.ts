...
test.describe('Create a new complete recipe (admin)', () => {
  test('Create a new recipe', async ({ adminContext }) => {
    ...
    // await page.fill('input[name="_recipe_settings[total_time]"]', '45');
...
test.describe('Favorite a recipe (contrib user)', () => {
  test('Favorite the recipe (frontend)', async ({ contribContext }) => {
    ...
    const initialCount = await contribPage.locator('.cooked-favorite-total').textContent();

    // Click the favorite heart
    await contribPage.locator('.cooked-favorite-heart').click();

    // Wait for the favorite action to complete and verify count increased
    await expect(contribPage.locator('.cooked-favorite-total')).toHaveText(String(Number(initialCount) + 1));

    // Verify the heart is now in "favorited" state (if there's a CSS class change)
    await expect(contribPage.locator('.cooked-favorite-heart')).toHaveClass(/cooked-is-favorite/);

    // Now unfavorite
    await contribPage.locator('.cooked-favorite-heart').click();

    // Verify count decreased back to original
    await expect(contribPage.locator('.cooked-favorite-total')).toHaveText(initialCount || '');

    // Verify the heart is back to unfavorited state
    await expect(contribPage.locator('.cooked-favorite-heart')).not.toHaveClass(/cooked-is-favorite/);
  });
});
