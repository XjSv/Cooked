...
test.describe('Admin Guest Ratings Settings', () => {
  test('Enable Guest and Star Ratings', async ({ adminContext }) => {
...
test.describe('Rate a recipe (anonymous user)', () => {
  test.use({ storageState: { cookies: [], origins: [] } });

  test('Rate the recipe (anonymous user)', async ({ page }) => {
    ...
    await page.locator('span.cooked-rating-stars .cooked-rating-choice[data-rating-value="3"]').click();

    // Add a small delay between ratings if needed
    await page.waitForTimeout(1000); // 1 second delay

    // Verify the average rating is 4.0
    await expect(page.locator('.cooked-current-rating')).toHaveText('3.0');

    // Now change to 5-star rating
    await page.locator('.cooked-rating-stars .cooked-rating-choice[data-rating-value="5"]').click();

    // Add a small delay between ratings if needed
    await page.waitForTimeout(1000); // 1 second delay

    // Verify the average rating is now 5.0
    await expect(page.locator('.cooked-current-rating')).toHaveText('5.0');
  });
});
