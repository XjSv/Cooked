...
test.describe('Admin Guest and Thumbs Up/Down Ratings Settings (admin user)', () => {
  test('Enable Guest and Thumbs Up/Down Ratings (admin user)', async ({ adminContext }) => {
    const adminPage = await adminContext.newPage();
    await adminPage.goto('/wp-admin/admin.php?page=cooked_settings#engagement', { waitUntil: 'networkidle' });

    await expect(adminPage.getByRole('checkbox', { name: 'cooked_settings[enable_guest_ratings][]' })).toBeDefined();

    // Check if the checkbox is checked first before clicking it.
    let switchery = adminPage.locator('input[name="cooked_settings[enable_guest_ratings][]"] ~ span.switchery');
    ...
    if (currentValue !== 'thumbs') {
      await select.selectOption('thumbs');
    }
...
test.describe('Guest Thumbs Up Rating Functionality (anonymous user)', () => {
  test.use({ storageState: { cookies: [], origins: [] } });

  test('Guest can submit thumbs up rating (anonymous user)', async ({ page }) => {
    if (!postId) {
    ...
    await page.locator('span.cooked-rating .cooked-icon-thumbs-up-solid').click({ force: true });
